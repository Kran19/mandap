    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          if (node.type == NodeType.stage || node.type == NodeType.carpet || node.type == NodeType.pole)
            _buildDimensionEditors(nodeId),
        ],
      ),
    );
}

Widget _buildDimensionEditors(NodeId nodeId) {
  final node = c.layout.getNode(nodeId);
  if (node == null) return const SizedBox();

  Widget buildField(String label, double? value, void Function(double?) onChanged) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  return Padding(
    ...
    child: Row(
      ...
      children: [
        if (node.type == NodeType.stage || node.type == NodeType.carpet || node.type == NodeType.pole) ...[
          buildField('Width (X)', node.width, (v) => c.updateNodeDimensions(nodeId: nodeId, newWidth: v, newDepth: node.depth, newHeight: node.height, newRotation: node.rotation, newElevation: node.elevation)),
          buildField('Depth (Z)', node.depth, (v) => c.updateNodeDimensions(nodeId: nodeId, newWidth: node.width, newDepth: v, newHeight: node.height, newRotation: node.rotation, newElevation: node.elevation)),
        ],
        if (node.type == NodeType.stage || node.type == NodeType.pole)
          buildField('Height (Y)', node.height, (v) => c.updateNodeDimensions(nodeId: nodeId, newWidth: node.width, newDepth: node.depth, newHeight: v, newRotation: node.rotation, newElevation: node.elevation)),
        if (node.type == NodeType.stage) ...[
          buildField('Elevation', node.elevation, (v) => c.updateNodeDimensions(nodeId: nodeId, newWidth: node.width, newDepth: node.depth, newHeight: node.height, newRotation: node.rotation, newElevation: v ?? 0.0)),
        ],
      ],
    ),
  );
