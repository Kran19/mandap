...
class ViewportTransform {
  ...
  /// Snap granularity in feet.
  static const double snapGrid = 0.5;
  ...
  Offset worldToScreen(double wx, double wz) {
    return Offset(wx * scale + panX, wz * scale + panY);
  }

  /// Converts a screen [Offset] to world coordinates (feet).
  ({double x, double z}) screenToWorld(Offset screen) {
    return (x: (screen.dx - panX) / scale, z: (screen.dy - panY) / scale);
  }

  /// Snaps a world coordinate pair to the nearest 0.5 ft grid.
  ({double x, double z}) snapToGrid(double wx, double wz) {
    return (
      x: (wx / snapGrid).round() * snapGrid,
      z: (wz / snapGrid).round() * snapGrid,
    );
  }
