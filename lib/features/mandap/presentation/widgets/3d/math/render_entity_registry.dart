...
class RenderEntityRegistry {
  ...
  NodeId? pickHandle({
    ...
    for (final handle in _handles.values) {
      final handle2D = v64.Vector3(handle.position.x, 0.0, handle.position.z);
      final dist = (target2D - handle2D).length;
      if (dist <= minDistance) {
        minDistance = dist;
